...
def _mbconv_layer(inputs,
    ...
    # Returns
        x (tensor): tensor as input to the next layer
    """
    conv = layers.SeparableConv2D(num_filters,
                         kernel_size=kernel_size,
                         strides=strides,
                         padding='same',
                         kernel_initializer='he_normal',
...
def resnet_mbconv(input_shape, depth, num_classes=10):
    ...
    for stack in range(3):
        for res_block in range(num_res_blocks):
            ...
            y = _mbconv_layer(inputs=x,
                              num_filters=num_filters,
                              strides=strides)
            y = _mbconv_layer(inputs=y,
                              num_filters=num_filters,
                              activation="relu")
            if stack > 0 and res_block == 0:  # first layer but not first stack
                # linear projection residual shortcut connection to match
                # changed dims
                x = _mbconv_layer(inputs=x,
                                  num_filters=num_filters,
                                  kernel_size=1,
                                  strides=strides,
                                  activation="relu",
...
def resnet_mbconv_v2(input_shape, depth, num_classes=10):
